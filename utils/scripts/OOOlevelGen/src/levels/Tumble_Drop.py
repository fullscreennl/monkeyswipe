...
def render(name,bg):
	...
	lb.addObject(Star.StarSprite(x=440,y=16,width=32,height=32, density=1))
	
	lb.addObject(Hero.HeroSprite(x=20,y=16))
	#lb.addObject(Friend.FriendSprite(x=20+2*spacing,y=16,width=32,height=32, density=1))
	
	lb.addObject(Enemy.EnemySprite(y=80,x=240,width=160,height=160,density='1',static='false'))
	lb.addObject(Enemy.EnemySprite(y=200,x=240,width=80,height=80,density='1',static='false'))
	lb.addObject(Enemy.EnemySprite(y=260,x=240,width=40,height=40,density='1',static='false'))
	lb.addObject(Enemy.EnemySprite(y=290,x=240,width=20,height=20,density='1',static='false'))
	lb.addObject(Enemy.EnemySprite(y=305,x=240,width=10,height=10,density='1',static='false'))
	lb.addObject(Enemy.EnemySprite(y=313,x=240,width=6,height=6,density='1',static='false'))
	#lb.addObject(SpikeyBuddy.SpikeyBuddySprite(x=240,y=160,width=32,height=32,density='1',restitution='0.7'))
	
	
	#static beams
	beamwidth = 100
	lb.addObject(Beam.BeamSprite(x=120,y=beamwidth/2,width=beamwidth,height=20,static='false',angle=90))
	lb.addObject(Beam.BeamSprite(x=120,y=beamwidth+beamwidth/2,width=beamwidth,height=20,static='false',angle=90))
