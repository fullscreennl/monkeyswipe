...
def render(name,bg):
	...
	lb.addObject(Star.StarSprite(x=445,y=60,width=40,height=40))
	lb.addObject(Beam.BeamSprite(x=380,y=0,width=120,height=120,static='true',angle=10))
	offset=76
	xpos = 406 #390
	lb.addObject(Nut.NutSprite(x=xpos,y=66,eventName='onNutHitAll'))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=64+offset,width=128,height=128))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=2*64+32+offset,width=64,height=64))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=2*(64+32)+16+offset,width=32,height=32))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=2*(64+32+16)+8+offset,width=16,height=16))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=2*(64+32+16+8)+4+offset,width=8,height=8))
	lb.addObject(Enemy.EnemySprite(x=xpos,y=2*(64+32+16+8+4)+2+offset,width=4,height=4))
	lb.addObject(Nut.NutSprite(x=15,y=160, eventName='onNutHitAll'))
	lb.addObject(Nut.NutSprite(x=205,y=300, eventName='onNutHitAll').setName('hinge'))
	lb.addObject(Nut.NutSprite(x=225,y=130, eventName='onNutHitAll'))
	lb.addObject(Nut.NutSprite(x=160,y=136, eventName='onNutHitAll'))
	lb.addObject(Nut.NutSprite(x=240,y=30, eventName='onNutHitAll'))
	lb.addObject(Beam.BeamSprite(x=205,y=320 - 144,width=120,height=12,static='false',angle=-10).setName('projectile')) 
	distJoint = Joints.DistanceJoint(body1='hinge',body2='projectile',damping='0.2',freq='0.8')
	lb.addObject(distJoint)
	lb.render()
