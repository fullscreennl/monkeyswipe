...
def render(name,bg):
	...
	lb.addObject(SpikeyBuddy.SpikeyBuddySprite(x=20+30,y=16,width=20,height=20, density=1))
	
	#static beams
	beamwidth = 320 - 160
	beamheight = 30
	lb.addObject(Beam.BeamSprite(x=380,y=beamwidth/2,width=beamwidth,height=beamheight,static='true',angle=90))
	
	lb.addObject(Star.StarSprite(x=440,y=16,width=32,height=32, density=1))
	for i in range(10):
		lb.addObject(Enemy.EnemySprite(y=160,x=420+(i%2)*40,width=32,height=32,static='false'))
	...
	distJoint = Joints.DistanceJoint(body1='Hero',body2='Spikey',damping='0.2',freq='1')
	lb.addObject(distJoint)
	#wipwap dyn part
